...
## Iteration log
- 2026-09-06: falsify 第106回。20:01 JST tick。worktree detached HEAD のため git fetch net-kotobase + checkout net-kotobase/main で同期確認 (HEAD 539f9a9 = rank 第103回, fetch 後 net-kotobase/main 先端一致, 奮離 0)。rank 第103回 NEXT「K-Z3 20時台帯初計測」に従い 20時台で実施 (cron 時刻 20:01 が20時台)。live smoke 200 (/, /signup; pre-run 計測)。host load1 39.06 (pre-run, gate 7.5 大幅超過) のため local 測定は拒否し production HTTP フォールバック (gate 外)。K-Z3 20時台帯初計測 run239A–C (同測定法 n=20 × 3 + landing control, 別接続 curl, 20:03:17–20:03:38 JST, 全 80/80 200, 正 endpoint search.kotobase.net/search?q=test): cold(>=0.5s) 0/0/0 per 20 = 0/60 完全静穏 — run239A p50 57.1ms max 107.8ms / run239B p50 49.6ms max 77.1ms / run239C p50 46.6ms max 131.6ms, control (kotobase.net/signup) cold 0/20 p50 52.2ms max 96.8ms 静穏で control 分離成立。20時台帯初計測は cold 0/60 完全静穏で日中低位帯パターン (13-17時台 2.0-5.0%) の下限側相当、18-19時台中間帯 (6.0-7.5%) から深夜帯平坦パターンへの遷移起点 (20-21時台帯別 ~17-58% 記録との対比) の判定材料として帯初 n=60 のみで水準確定は次の 1-2 セットに委ねる。status 遷移なし (rank 専門)。secret は一切記録せず (curl のみ)。NEXT: 委ねる (rank 指定優先; フォールバックは K-Z3 20時台 n 積み増し継続)。
