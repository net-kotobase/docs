...
rank (期待 gain × 確率, 2026-09-04 第20回):
...
2. K-Z3 — 時間帯別発現率分布。午前帯 5/14 → 昼過ぎ〜午後開始帯 (run18–24) 4/7 →
   ...
   run64–66 (14:29–30) は 3/3 試行連続 cold>0 (run4–6 以来初) だが
   landing page control (14:38: p50 280ms, cold 2/20) も上振れしており
   host load1 ~55–67 帯では local/host 由素混入を排除できず not-separated。
   verdict 付きの確定数値としては蓄積せず、分布への採用は控える。
