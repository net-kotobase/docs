...
rank (期待 gain × 確率, 2026-09-04 第18回):
...
2. K-Z3 — 時間帯別発現率分布。午前帯 5/14 → 昼過ぎ〜午後開始帯 (run18–24) 4/7 →
   ...
   (run52–59) 2/8 → run60 (14:11) cold 4/20 + warm 群同時上振れ (p50 217ms) の
   run4–6 型突発 3 例目 — 13:35–14:01 の静穏帯 (0/11, p50 60–90ms) が再び崩れた。
   昼帯〜午後帯通算 cold>0 は 60 試行中 26 試行 (~43%)。run4–6 型は静穏帯の後の
   突発で再出現しており、単発型への収束説は後退。夕方帯の n 積み増しが次。
