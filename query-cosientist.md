...
rank (期待 gain × 確率, 2026-09-04 第12回):
1. K-Z2 — 日中帯 cold 群の短時間スケール再発の機構切分け。発火直後 vs 経過後対比
   (3 組中 2 組で直後のみ cold 群) を維持。*/2 高頻度化の介入は反証まで保留のまま
   (run4–6 型再発は突発的: 昼帯 falsify run25–27 が 3/3 試行で cold>0 の直後、
   bench run28–30 が同窓で 0/20×3 — 時間窓内でも連続しない。午前〜昼帯通算
   cold>0 は 27 試行中 12 試行)。実効最上位 (gate 外で観測継続可能)。
2. K-Z3 — 時間帯別発現率分布。午前帯 5/14 → 昼過ぎ〜午後開始帯 (run18–24) 4/7 →
   昼帯 (run25–27) 3/3、run28–30 は 0/3。午前〜昼帯通算 27 試行中 12 試行 (~44%)。
   夕方帯の n 積み増しが */2 判断の直接の証拠。K-Z2 と観測を共有。
3. K-Q1 — 恒常的 query path 退行の切り分け。残る切れ手は verify-session 1 重化
   hand-patch の local 効果予測だが、host load1 57–64 (gate 7.5 超過継続) で
   local 測定の見込みが続かず停滞中。
