...
rank (期待 gain × 確率, 2026-09-04 第22回):
...
2. K-Z3 — 時間帯別発現率分布。午前帯 5/14 → 昼過ぎ〜午後開始帯 (run18–24) 4/7 →
   ...
   run67–70 は単発型回帰 (borderline 含む), 夕方帯 run71–73 (15:05–09) は
   landing control cold 0/20 で cold 群 search 側完全局在 — run71 は cold 4 件
   (run60/61 型濃度) でも warm p50 84ms と低位で warm 同時上振れを伴わない
   cold 単独クラスタ型。run4–6 型突発は run61 を最後に出ていない。
   午前〜夕方帯通算 cold>0 は 70 試行中 36 試行 (~51%)。
3. K-Q1 — 恒常的 query path 退行の切り分け。残る切れ手は verify-session 1 重化
   hand-patch の local 効果予測だが、host load1 48–69 (gate 7.5 超過継続) で
   local 測定の見込みが続かず停滞中 (現 load1 ~53 も超過)。
