...
rank (期待 gain × 確率, 2026-09-04 第16回):
1. K-Z2 — 日中帯 cold 群の短時間スケール再発の機構切分け。発火直後 vs 経過後対比
   (3 組中 2 組で直後のみ cold 群) を維持。*/2 高頻度化の介入は反証まで保留のまま
   (発現は突発的で時間窓内でも連続しない — run49–50 の単発 cold 群は run46–48 の
   0/3 直後の数分で出現し 1 試行で消失)。実効最上位 (gate 外で観測継続可能)。
2. K-Z3 — 時間帯別発現率分布。午前帯 5/14 → 昼過ぎ〜午後開始帯 (run18–24) 4/7 →
   ...
   昼帯〜午後帯通算 cold>0 は 48 試行中 22 試行 (~46%: falsify 昼帯通算 20/45 +
   bench run49–51 2/3) で午前帯 (~36%) より高位。run49–50 は 1.0–1.3s 帯の単発型
   (warm 群遅延上振れなし) で run13–16/run37 型に整合し、run4–6 型 (warm 群同時
   上振れ) は run32–33 を最後に出ていない。夕方帯の n 積み増しが次。
